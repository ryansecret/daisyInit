...
module.exports = class Command {
    ...
    // ask for target dir
     ...
     let gitInfo={
      ...
      if(answer.isCommon)
      {
          ...
          if(answer.templateType==='ts')
          {
              gitInfo.branch='feature-typeScript'
          }
          if(answer.templateType==='node')
          {
      ...
      {
          gitInfo.origin='gitlab:git.jd.com:chenjingnan/template'
          answer = await this.inquirer.prompt({
              type:'list',
              name: 'templateType',
              message: '请选择模板类型: ',
  ...
  async download(targetPath, gitInfo) {
    try {
        ...
        let origin = this.argv.origin||gitInfo.origin ||`github.com:ryansecret/tsTemplate`
        let branch =`${this.argv.branch}`||gitInfo.branch || 'template'
